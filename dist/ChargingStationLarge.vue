<template>
                            <svg :width="width" :height="height" viewBox="0 0 48 48" version="1.1" xmlns="http://www.w3.org/2000/svg">
    <title>
        {{title}}
    </title>
    <path d="M29,44.5 L3,44.5 M26.5,44.5 L26.5,6.5 C26.5,4.84314575 25.1568542,3.5 23.5,3.5 L8.5,3.5 C6.84314575,3.5 5.5,4.84314575 5.5,6.5 L5.5,44.5 L5.5,44.5 M24,20.5 L8,20.5 M40.5,21.5 L40.5,35.5 C40.5,37.709139 38.709139,39.5 36.5,39.5 C34.290861,39.5 32.5,37.709139 32.5,35.5 L32.5,24.5 C32.5,22.290861 30.709139,20.5 28.5,20.5 L26.5,20.5 L26.5,20.5 M46.5,9.5 L46.5,15.5 C46.5,18.8137085 43.8137085,21.5 40.5,21.5 C37.1862915,21.5 34.5,18.8137085 34.5,15.5 L34.5,9.5 L34.5,9.5 L46.5,9.5 Z M37.5,9.5 L37.5,4.5 M43.5,9.5 L43.5,4.5" stroke="currentColor" stroke-width="1" fill="none" fill-rule="evenodd"/>
</svg>
</template>
                            <script lang="ts">
                                import { Component, Prop, Vue } from "vue-property-decorator";
                                
                                @Component
                                export default class ChargingStationLarge extends Vue {
                                    @Prop({default: 48}) width!:number;
                                    @Prop({default: 48}) height!:number;
                                    @Prop({default: 'charging-station-large'}) title!:string;
                                }
                            </script> 