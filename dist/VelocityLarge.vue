<template>
                            <svg :width="width" :height="height" viewBox="0 0 48 48" version="1.1" xmlns="http://www.w3.org/2000/svg">
    <title>
        {{title}}
    </title>
    <path d="M21.5,7.6 L21.5,4.5 L21.5,7.6 Z M25.5,7.6 L25.5,4.5 L25.5,7.6 Z M16.5,1.5 L30.5,1.5 L30.5,4.5 L16.5,4.5 L16.5,1.5 Z M37.6,14.3 L40.5,11.4 L37.7,8.6 L34.8,11.5 M12.3,11.4 L9.4,8.5 L6.6,11.3 L9.5,14.2 M23.5,28.5 C25.1568542,28.5 26.5,27.1568542 26.5,25.5 C26.5,23.8431458 25.1568542,22.5 23.5,22.5 C21.8431458,22.5 20.5,23.8431458 20.5,25.5 C20.5,27.1568542 21.8431458,28.5 23.5,28.5 Z M26.3,26.5 L34.8,29.6 M23.5,43.5 C33.4411255,43.5 41.5,35.4411255 41.5,25.5 C41.5,15.5588745 33.4411255,7.5 23.5,7.5 C13.5588745,7.5 5.5,15.5588745 5.5,25.5 C5.5,35.4411255 13.5588745,43.5 23.5,43.5 Z" stroke="currentColor" stroke-width="1" fill="none" fill-rule="evenodd"/>
</svg>
</template>
                            <script lang="ts">
                                import { Component, Prop, Vue } from "vue-property-decorator";
                                
                                @Component
                                export default class VelocityLarge extends Vue {
                                    @Prop({default: 48}) width!:number;
                                    @Prop({default: 48}) height!:number;
                                    @Prop({default: 'velocity-large'}) title!:string;
                                }
                            </script> 